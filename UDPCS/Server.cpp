...
DWORD WINAPI WorkerThread(LPVOID val)
{
	...
	if (sendto(conn->socket, conn->sendBuf, strlen(DOCHIRA), 0, (struct sockaddr *)&conn->client, conn->clientLen) != n)
	{
		perror("sendto error");
		return 1;
	}
	...
	if (sendto(conn->socket, conn->sendBuf, strlen(O_NKGX), 0, (struct sockaddr *)&conn->client, conn->clientLen) != n)
	{
		perror("sendto error");
		return 1;
	}
